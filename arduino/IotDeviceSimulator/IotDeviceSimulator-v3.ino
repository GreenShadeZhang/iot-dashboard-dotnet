 ...
 * - Light control (LED)
 * - Servo control (0-180 degrees, controlled via fan speed 0-100)
 * - Battery level monitoring
 * - Serial communication using custom protocol
 * 
 * Hardware connections:
 * - LED on pin 15
 * - Servo on pin 14 (controlled via fan speed command)
...
// Pin definitions
#define LED_PIN 15          // Light control pin
#define FAN_PIN 14          // Servo control pin (0-180 degrees)
#define BUILTIN_LED_PIN 12  // Built-in LED for status (ESP32: use LED_BUILTIN or 2)
#define DHT_PIN 27          // DHT sensor pin
...
#define UPDATE_INTERVAL 1000 // Update sensors every second

// Battery simulation constants
#define BATTERY_DRAIN_MAX 2
#define BATTERY_RESET_THRESHOLD 20
#define BATTERY_RESET_VALUE 100
...
void setup() {
  ...
  // Initialize servo
  QDServo.attach(FAN_PIN, 500, 2500);
  ...
  QDServo.write(0); // Set servo to 0 degrees
...
void updateSensors() {
  ...
  // Update state if reading is successful (result == 0)
  if (result == 0) {
  ...
  switch (cmdType) {
    ...
    case CMD_SET_FAN_SPEED:
      if (dataLength >= 1) {
        ...
        // Map fan speed (0-100) to servo angle (0-180)
        int servoAngle = map(state.fanSpeed, 0, 100, 0, 180);
        QDServo.write(servoAngle);
        sendState();
      }
      break;
    ...
    case CMD_RESET:
      ...
      state.fanSpeed = 0;
      digitalWrite(LED_PIN, LOW);
      QDServo.write(0); // Reset servo to 0 degrees
      sendState();
      break;
